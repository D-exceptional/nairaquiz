...
<head>
  ...
  <title>Admin | Game Approvals</title>
...
<body class="hold-transition sidebar-mini">
        ...
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1>
              <b>
                <?php
                  // Convert date to MySQL DATETIME format (Y-m-d H:i:s)
                  $inputDate = '11/20/2024 16:00'; // MM/DD/YYYY HH:MM
                  $dateTime = DateTime::createFromFormat('m/d/Y H:i', $inputDate);
                  $date = $dateTime ? $dateTime->format('Y-m-d H:i:s') : '1970-01-01 00:00:00'; // Fallback in case parsing fails
              
                  $statusPending = 'Pending';
              
                  // Prepare the statement
                  $stmt = $conn->prepare("SELECT SUM(fund_amount) AS total_amount FROM wallet_fund WHERE fund_status = ? AND fund_date != ?");
                  
                  if (!$stmt) {
                    die("Prepare failed: " . $conn->error); // Debug prepare error
                  }
              
                  $stmt->bind_param("ss", $statusPending, $date);
                  ...
                  echo "Game Approvals ($formatted_amount)";
                ?>
              </b>
            </h1>
          </div>
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Game Approvals</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <section class="content" style='overflow-y: auto !important;box-sizing: border-box;'>
      ...
      <div class="card" style='height: 550px;'>
        <div class="card-header">
          <h3 class="card-title">Game Approvals</h3>
        ...
        <div class="card-body p-0" style='overflow-x: auto !important;white-space: normal;'>
          <table class="table table-striped projects">
            ...
            <tbody>
              <?php
                $excluded_date = '11/20/2024 16:00';
                $status = 'Pending';
                
                $stmt = $conn->prepare("SELECT * FROM wallet_fund WHERE fund_status = ? AND fund_date != ? ORDER BY fundID DESC");
                $stmt->bind_param("ss", $status, $excluded_date);
                $stmt->execute();
                $result = $stmt->get_result();
                
                if ($result->num_rows > 0) {
                  // Prepare secondary statements once, outside the loop
                  $receipt_stmt = $conn->prepare("SELECT receipt_image FROM wallet_fund_receipt WHERE receipt_name = ?");
                  $user_stmt = $conn->prepare("SELECT fullname, email FROM users WHERE userID = ?");
                  $currency_stmt = $conn->prepare("SELECT wallet_currency FROM wallet WHERE userID = ?");
                
                  while ($value = $result->fetch_assoc()) {
                    $fund_id = $value['fundID'];
                    $user_id = $value['userID'];
                    $amount = $value['fund_amount'];
                    $fund_date = $value['fund_date'];
                    ...
                    $receipt_name = $receipt_data['receipt_image'] ?? 'no-receipt.png';
                    ...
                    // Currency
                    $currency_stmt->bind_param("i", $user_id);
                  ...
                  echo "<tr><td colspan='10'><p style='text-align: center;'>No pending payments available</p></td></tr>";
                }
  ...
  <script src="<?php echo getCacheBustedUrl('../scripts/game-approval.js'); ?>" type="module"></script>

...
<head>
  ...
  <title>Admin | Investment Pay-ins</title>
        ...
        <div class="row mb-2">
            ...
            <h1>
                <b>
                    <?php
                        ...
                        echo "Investment Pay-ins ($formatted_amount)";
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">Investment Pay-ins</li>
            </ol>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    ...
    <section class="content" style='overflow-y: auto !important;box-sizing: border-box;'>
      ...
      <div class="card" style='height: 550px;'>
        <div class="card-header">
          <h3 class="card-title">Investment Pay-ins</h3>
          ...
          <table class="table table-striped projects">
              ...
              <tbody>
                <?php
                    ...
                    $fund_stmt = $conn->prepare("SELECT * FROM investor_plans ORDER BY planID DESC");
                    $fund_stmt->execute();
                    $fund_result = $fund_stmt->get_result();
                    
                    if ($fund_result->num_rows > 0) {
                        // Prepare the investor info query once
                        $investor_stmt = $conn->prepare("SELECT fullname FROM investors WHERE investorID = ?");
                    
                        while ($value = $fund_result->fetch_assoc()) {
                            $investorID = $value['investorID'];
                            $amount = $value['plan_amount'];
                            $date = $value['plan_created'];
                            $reference = $value['plan_reference'];
                            $status = $value['plan_status'];
                    
                            // Format amount
                            $payment_amount = "&#x20A6;" . number_format($amount, 2, '.', ',');
                    
                            // Get user's fullname
                            $investor_stmt->bind_param("i", $investorID);
                            $investor_stmt->execute();
                            $investor_result = $investor_stmt->get_result();
                            $investor_data = $investor_result->fetch_assoc();
                            $fullname = $investor_data['fullname'] ?? 'Unknown';

...
if (!empty($currentPassword) && !empty($newPassword)) {
    ...
    $stmt = $conn->prepare("SELECT user_password FROM users WHERE userID = ? AND user_type = 'Admin'");
    ...
    if ($stmt->fetch()) {
        ...
        if (password_verify($currentPassword, $dbPassword)) {
            ...
            // Update password
            $updateStmt = $conn->prepare("UPDATE users SET user_password = ? WHERE userID = ? AND user_type = 'Admin'");

      ...
      <li class="nav-item dropdown">
        <a class="nav-link" data-toggle="dropdown" href="#" id="notification-link">
          <i class="far fa-bell"></i>
           <?php 
               $notification_count = mysqli_query($conn, "SELECT * FROM investor_notifications WHERE notification_receiver = '$email' AND notification_status = 'Unseen' GROUP BY notification_type");
        ...
        <?php 
            $incoming_mail_count = mysqli_query($conn, "SELECT * FROM investor_notifications WHERE notification_type = 'incoming_mail' AND notification_receiver = '$email' AND notification_status = 'Unseen'");
            if (mysqli_num_rows($incoming_mail_count) > 0) {
               $total = mysqli_num_rows($incoming_mail_count);
               //Get last notification item time
               $last_time = mysqli_query($conn, "SELECT notification_date FROM investor_notifications WHERE notification_type = 'incoming_mail' AND notification_receiver = '$email' AND notification_status = 'Unseen' ORDER BY notificationID DESC LIMIT 1");
               $value = mysqli_fetch_assoc($last_time);
               $notification_date = $value['notification_date'];
               $counter = timeAgo($notification_date);
               echo "
                  <a href='#' class='dropdown-item' style='font-size: 0.9rem;'>
         ...
         <?php 
            $user_registration_count = mysqli_query($conn, "SELECT * FROM investor_notifications WHERE notification_type = 'user_registration' AND notification_receiver = '$email' AND notification_status = 'Unseen'");
            if (mysqli_num_rows($user_registration_count) > 0) {
               $total = mysqli_num_rows($user_registration_count);
               //Get last notification item time
               $last_time = mysqli_query($conn, "SELECT notification_date FROM investor_notifications WHERE notification_type = 'user_registration' AND notification_receiver = '$email' AND notification_status = 'Unseen' ORDER BY notificationID DESC LIMIT 1");
               $value = mysqli_fetch_assoc($last_time);
               $notification_date = $value['notification_date'];
               $counter = timeAgo($notification_date);
               echo "
